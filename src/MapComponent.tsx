...
export const MapComponent: React.FC<MapComponentProps> = ({
	...
	const groups = Array.from(new Set(pins.map(pin => pin.group).filter(Boolean))) as string[];

	useEffect(() => {
		if (!mapRef.current || olMapRef.current) return;

		// Filter pins based on hidden groups
		const visiblePins = pins.filter(pin => !pin.group || !hiddenGroups.has(pin.group));
		
		// Calculate optimal view for visible pins
		const optimalView = visiblePins.length > 1 ? calculateBounds(visiblePins) : { center: initialCenter, zoom: initialZoom };
	...
	}, [pins, initialCenter, initialZoom, pinSize, defaultPinColor, hiddenGroups]);
